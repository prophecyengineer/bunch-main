...
export default function Username() {
  ...
  const [usernameCharacterChecker, setUsernameCharacterChecker] =
    useState(true);
  ...
  const handleUsernameChange = async (username: string) => {
    if (username.match(format)) {
      setUsernameCharacterChecker(false);
    } else {
      setUsernameCharacterChecker(true);
    }
  ...
  const form = useForm({
    ...
    validate: {
      username: (value) => (value.length < 3 ? "name-error" : null),
    },
  });
  ...
  return (
    <>
      <Portal zIndex={5}>
        <Transition
          mounted={!usernameCharacterChecker}
          transition="slide-up"
          duration={400}
          timingFunction="ease"
        >
          {(styles) => (
            <div style={{ ...styles, ...padding }}>
              <Notification icon={<X size={18} />} color="red" disallowClose>
                the symbol <b>{username.match(format)}</b> is not allowed
              </Notification>
            </div>
          )}
        </Transition>
